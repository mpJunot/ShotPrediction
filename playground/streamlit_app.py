...
def main():
    ...
    with st.sidebar:
        ...
        # Phase detection model option
        enable_phase_detection = st.checkbox(
            "Enable Shot Phase Detection",
            value=True,
            help="Use copyme.pt model to detect different phases of a shot"
        )

        phase_model_path = None
        if enable_phase_detection:
            default_phase_model_path = os.path.join(project_root, 'models', 'copyme.pt')
            phase_model_path = st.text_input(
                "Phase Detection Model Path",
                value=default_phase_model_path,
                help="Path to the shot phase detection model (copyme.pt)"
            )
        ...
        if video_source is not None:
            ...
            with col_video:
                ...
                process_video(
                    ...
                    enable_phase_detection, phase_model_path
                )
...
def process_video(video_source, model_path, fps, px_per_meter, min_speed_threshold,
                 ...
                 enable_phase_detection=True, phase_model_path=None):
    """Process video with basketball analyzer"""

    try:
        # Initialize analyzer
        analyzer_key = f"analyzer_{model_path}_{fps}_{px_per_meter}_{enable_phase_detection}"
        if (analyzer_key not in st.session_state or
            st.session_state.get('model_path') != model_path or
            st.session_state.get('enable_phase_detection') != enable_phase_detection):
            with st.spinner("Loading model..."):
                ...
                st.session_state.enable_phase_detection = enable_phase_detection
                st.session_state.current_analyzer_key = analyzer_key
                if enable_phase_detection:
                    st.success("Models loaded successfully! (Basic + Phase Detection)")
                else:
        ...
        while True:
            ...
            # Add phase detection info if available
            if 'current_phase' in stats:
                current_stats['current_phase'] = stats['current_phase']
                current_stats['current_phase_name'] = stats.get('current_phase_name', 'Unknown')
                current_stats['phase_history_count'] = stats.get('phase_history_count', 0)
                current_stats['has_required_phases'] = stats.get('has_required_phases', False)
                current_stats['is_in_active_phase'] = stats.get('is_in_active_phase', False)
...
def display_realtime_metrics():
    ...
    # Calculate maximum probability from all predictions
    max_probability = stats['probability']
    if 'stats_history' in st.session_state and len(st.session_state.stats_history) > 0:
        all_probs = [s['probability'] for s in st.session_state.stats_history]
        if all_probs:
            max_probability = max(all_probs)

    prob_color = "status-success" if max_probability > 0.6 else "status-warning" if max_probability > 0.3 else "status-error"
    prob_label = "Peak Probability"
    st.markdown(f"""
    <div class="metric-card">
        <h4>{prob_label}</h4>
        <h2 class="{prob_color}">{max_probability*100:.1f}%</h2>
    </div>
    ...
    if 'current_phase' in stats and stats['current_phase'] is not None:
        st.markdown("---")
        st.markdown("### Shot Phase")
        phase_name = stats.get('current_phase_name', f"Phase {stats['current_phase']}")
        st.info(f"Current Phase: {phase_name}")
        if 'phase_history_count' in stats:
            st.caption(f"Phase detections: {stats['phase_history_count']}")
        from basketball_analyzer.config import PHASE_DETECTION_CONFIDENCE_THRESHOLD
        st.caption(f"Confidence threshold: {PHASE_DETECTION_CONFIDENCE_THRESHOLD:.1%}")
...
def display_about_page():
    ...
    **Shot Phase Detection Model (copyme.pt):**
    - Class 0: shot_position - Player positioning and preparation
    - Class 1: shot_release - Ball release phase
    - Class 2: shot_followthrough - Follow-through motion
